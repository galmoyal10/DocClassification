...
public class Result {
	...
	static Double[] compare(Result result, Result expected) {
		...
		s1.retainAll(s2);
		Double precision =  (double)result.documents.length == 0 ? 0 : ((double)s1.size() / (double)result.documents.length);
		Double recall = ((double)s1.size() / (double)expected.documents.length);
